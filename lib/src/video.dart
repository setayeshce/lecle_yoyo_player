...
class _YoYoPlayerState extends State<YoYoPlayer> with SingleTickerProviderStateMixin {
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          gradient: LinearGradient(
                            ...
                            colors: [Colors.black.withOpacity(0.5600000023841858), Colors.white.withOpacity(0.1), Colors.black.withOpacity(0.6399999856948853)],
                          ),
                        ),
                      ),
                    ],
  ...
  int _speedIndex = 0;
  final List<double> _playbackSpeeds = [0.75, 0.5, 1.0];
  void changePlaybackSpeed() {
    // Update the speed index to the next speed
    _speedIndex = (_speedIndex + 1) % _playbackSpeeds.length;

    // Get the new speed based on updated index
    double newSpeed = _playbackSpeeds[_speedIndex];

    // Set the new playback speed
    controller.setPlaybackSpeed(newSpeed);

    // Optionally, you might want to show a toast or some UI feedback about the current speed
    print("Playback speed changed to $newSpeed");

    // Ensure the widget reflects the change (if displaying the speed somewhere)
    setState(() {});
  }
  ...
  Future<M3U8s?> m3u8Video(String? videoUrl) async {
    ...
    if (m3u8Content == null && videoUrl != null) {
      ...
      if (response.statusCode == 200) {
        ...
        print("--- m3u8 File write --- ${yoyo.map((e) => e.dataQuality == e.dataURL).toList()} --- length : ${yoyo.length} --- Success");
        return m3u8s;
      }
    }
...
// Video listener
  ...
  void m3u8Clean() async {
    ...
    for (int i = 2; i < yoyo.length; i++) {
      try {
        var file = await FileUtils.readFileFromPath(videoUrl: yoyo[i].dataURL ?? '', quality: yoyo[i].dataQuality ?? '');
